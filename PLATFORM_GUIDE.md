...
- Кнопки по ситуации:
  ...
  - **Пожаловаться** - сигнал всем администраторам (админ откроет предложение глазами
    автора жалобы).
- **💬 Чат** - личная переписка двух участников предложения прямо на этой странице
  (кнопка «Чат»): текстовые сообщения и вложения (PNG/JPG/PDF), время отправки и
  отметка о прочтении (галочки). Новые сообщения прилетают мгновенно по тому же
  SSE-потоку, что и уведомления. Счётчик непрочитанных виден на кнопке «Чат», на
  карточках в ленте предложений и на пункте меню «Предложения».
...
Сервер построен из 15 независимых модулей. Ниже - что делает каждый, человеческим языком.
...
### 7.12 💬 Chat - личные чаты по предложениям обмена

- «Диалог» привязан к офферу: его участники - это стороны предложения
  (`proposerId`/`recipientId`), отдельной таблицы диалога нет.
- Сообщения: текст и/или вложения (PNG/JPG/PDF). Время и факт прочтения - на каждом
  сообщении (`readAt` проставляется, когда вторая сторона открыла чат).
- **Вложения - приватные**: отдаются только участникам через авторизованный
  эндпоинт (не публично, в отличие от фото лотов). Лимиты: изображение до 8 МБ,
  PDF до 16 МБ, до 5 вложений на сообщение.
- **Антивирус** - подключаемый порт (`FileScanner`): по умолчанию выключен (файлы
  сохраняются без проверки - удобно для dev/offline), при `ANTIVIRUS_ENABLED=true`
  каждый файл проверяется демоном ClamAV по TCP (INSTREAM) ещё до сохранения;
  заражённый файл отклоняется, сбой проверки трактуется как недоступность.
- **Реальное время** - переиспользует SSE-поток уведомлений (Redis pub/sub): новое
  сообщение и отметка о прочтении долетают мгновенно, отдельного соединения нет.
- Уведомление о новом сообщении **коалесцируется**: пока получатель не открыл диалог,
  на оффер держится одно уведомление в колокольчике (обновляется, не спамит).
...
| Имя                                   | 5–200 символов                                 |
| Вложение чата - изображение           | **до 8 МБ** (PNG/JPG)                          |
| Вложение чата - документ              | **до 16 МБ** (PDF)                             |
| Вложений в одном сообщении            | **до 5**                                       |
| Длина текста сообщения чата           | до 4000 символов                               |
...
| Событие                     | Кто получает                                                              |
| --------------------------- | ------------------------------------------------------------------------ |
| 🔔 Новое сообщение в чате   | второй участник предложения (коалесцируется: одно уведомление на оффер до прочтения) |
...
> SSE-поток уведомлений также доставляет события чата (`chat:message`, `chat:read`,
> `chat:unread`) — отдельного соединения для чата нет.

</details>

<details>
<summary><b>💬 Чат - <code>/chat</code></b></summary>

| Метод | Путь                                       | Что делает                                       |
| ----- | ------------------------------------------ | ------------------------------------------------ |
| GET   | `/chat/offers/:offerId/messages`           | сообщения диалога (участник; cursor-пагинация)   |
| POST  | `/chat/offers/:offerId/messages`           | отправить сообщение (текст и/или вложения)        |
| POST  | `/chat/offers/:offerId/attachments`        | загрузить вложение PNG/JPG/PDF (антивирус)        |
| POST  | `/chat/offers/:offerId/read`               | пометить входящие прочитанными                    |
| GET   | `/chat/offers/:offerId/file/:attachmentId` | скачать приватное вложение (только участник)       |
| GET   | `/chat/unread-count`                       | суммарно непрочитанных (бейдж навигации)          |
| GET   | `/chat/unread/by-offer`                    | непрочитанные по офферам (бейджи карточек)         |
...
Перед первым запуском создайте конфигурацию из шаблонов: `server/.env.example` →
`server/.env` и `client/.env.example` → `client/.env` (назначение переменных —
в комментариях шаблонов). После этого вся система поднимается одной командой
через Docker:
...
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 💬 **Чаты между участниками обмена** (вложения PNG/JPG/PDF, время и прочтение, real-time по SSE) | ✅ готово; антивирус — подключаемый ClamAV (по умолчанию выключен)        |
